...
<head>
  ...
  <title>PANOBOOKING | Ambulances Records</title>
            ...
            <li class="nav-item menu-open">
              ...
              <ul class="nav nav-treeview">
                ...
                <li class="nav-item">
				 ...
				 <li class="nav-item">
                  <a href="ambulances.php" class="nav-link active">
                    <i class="far fa-circle nav-icon"></i>
            ...
            <div class="col-12">
              ...
              <div class="card">
                <div class="card-header">
                  <h3 class="card-title">Ambulances Records</h3>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                  <table id="example1" class="table table-bordered table-striped">
                    <thead>
                      <tr style='overflow-x:scroll'>
                        ...
                        <th>Ambulance Specifications</th>
                        <th>Driver Name</th>
                        <th>Driver Age</th>
                        <th>Ratings</th>
                        <th>Email Address</th>
                    ...
                    <tbody>
                      ...
                      $sales7 = "SELECT * FROM `ambulances` where DriverName='$places' order by IDs Desc";
                      ...
                      while ($row7 = mysqli_fetch_assoc($result7)) {
                        ...
                        $id = $row7['IDs'];
                        echo  "<td>" . $counter3 . "</td>";
                        echo  "<td style='overflow-x:scroll;'>" . $row7['TaxiNo'] . "</td>";
                        echo  "<td style='overflow-x:scroll;'>" . $row7['CarSpec'] . "</td>";
                        echo  "<td style='overflow-x:scroll;'>" . $row7['DriverName'] . "</td>";
                        echo  "<td style='overflow-x:scroll;'>" . $row7['DriverAge'] . "</td>";
                        echo  "<td style='overflow-x:scroll;'>" . $row7['Ratings'] . "</td>";
                        echo  "<td style='overflow-x:scroll;'>" . $row7['EmailAddress'] . "</td>";
                        echo  "<td style='overflow-x:scroll;'>" . $row7['TelephoneNumber'] . "</td>";
                        echo  "<td style='overflow-x:scroll;'>" . $row7['Price'] . "</td>";
                        echo  "<td style='overflow-x:scroll;'>" . $row7['Repayments'] . "</td>";
                        echo  "<td style='overflow-x:scroll;'>" . $row7['CurrentLocation'] . "</td>";
                        echo  "<td style='overflow-x:scroll;'>" . $row7['Availability'] . "</td>";
						if($row7['Offered']=="Not Offered"){
						echo  "<td><form method='get' action='ambulances.php'><button type='submit' name='offer'  value='" . $id . "' class='btn btn-block btn-warning'>Offer</button></form></td>";
					  }else{
						echo  "<td><form method='get' action='ambulances.php'><button type='submit' name='notoffer'  value='" . $id . "' class='btn btn-block btn-warning'>Not Offered</button></form></td>";
					  }
						if($row7['Approved']=="Not Approved"){
						echo  "<td><form method='get' action='ambulances.php'><button type='submit' name='approve'  value='" . $id . "' class='btn btn-block btn-warning'>Approve</button></form></td>";
					  }else{
						  echo  "<td style='overflow-x:scroll;'>" . $row7['Approved'] . "</td>";  
					  }
                        echo  "<td><form method='get' action='ambulances.php'><button type='submit' name='delete'  value='" . $id . "' class='btn btn-block btn-danger'>Delete</button></form></td>";
                        echo " </tr>";
                        $counter3++;
                      }
						 }else{
                      $dates = date("Y-m-d");;
                      require_once('../../connect/connect.php');
                      $sales7 = "SELECT * FROM `ambulances` order by IDs Desc";
                      $result7 = mysqli_query($link, $sales7);
                      $counter3 = 1;
                      $totalstock = 0;
                    ...
                    if (isset($_GET['approve'])) {
                      $id = $_GET['approve'];
                      $sales7 = "UPDATE  ambulances SET Approved='Approved' WHERE IDs='" . $id . "'";
                      $result = mysqli_query($link, $sales7);
                      if ($result) {
                        echo '<script type="application/javascript">';
                        echo 'alert("Ambulance Record successfully Approved");';
                        echo 'window.location.href="ambulances.php";';
                        echo '</script>';
                      } else {
                        echo '<script type="application/javascript">';
                        echo 'alert("Ambulance Approval Failed");';
                        echo 'window.location.href="ambulances.php";';
                        echo '</script>';
                      }
                    }
					 if (isset($_GET['offer'])) {
                      $id = $_GET['offer'];
                      $sales7 = "UPDATE  ambulances SET Offered='Offered' WHERE IDs='" . $id . "'";
                      $result = mysqli_query($link, $sales7);
                      if ($result) {
                        echo '<script type="application/javascript">';
                        echo 'alert("Ambulance Record successfully Offered");';
                        echo 'window.location.href="ambulances.php";';
                        echo '</script>';
                      } else {
                        echo '<script type="application/javascript">';
                        echo 'alert("Ambulance Offer Failed");';
                        echo 'window.location.href="ambulances.php";';
                        echo '</script>';
                      }
                    }
					if (isset($_GET['notoffer'])) {
                      $id = $_GET['notoffer'];
                      $sales7 = "UPDATE  ambulances SET Offered='Not Offered' WHERE IDs='" . $id . "'";
                      $result = mysqli_query($link, $sales7);
                      if ($result) {
                        echo '<script type="application/javascript">';
                        echo 'alert("Ambulance offfer Record successfully Removed ");';
                        echo 'window.location.href="ambulances.php";';
                        echo '</script>';
                      } else {
                        echo '<script type="application/javascript">';
                        echo 'alert("Ambulance Offer Removal Failed");';
                        echo 'window.location.href="ambulances.php";';
                        echo '</script>';
                      }
                    }
					if (isset($_GET['delete'])) {
                      $id = $_GET['delete'];
                      $sales7 = "DELETE  FROM ambulances WHERE IDs='" . $id . "'";
                      $result = mysqli_query($link, $sales7);
                      if ($result) {
                        echo '<script type="application/javascript">';
                        echo 'alert("Ambulance Record successfully Deleted");';
                        echo 'window.location.href="ambulances.php";';
                        echo '</script>';
                      } else {
                        echo '<script type="application/javascript">';
                        echo 'alert("Ambulance Record Delete Failed");';
                        echo 'window.location.href="ambulances.php";';
                        echo '</script>';
                      }
                    }
