...
<head>
  ...
  <title>PANOBOOKING | Airport Taxis Records</title>
          ...
          <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
            ...
            <li class="nav-item menu-open">
              ...
              <ul class="nav nav-treeview">
                ...
                <li class="nav-item">
                  <a href="taxis.php" class="nav-link active">
                    <i class="far fa-circle nav-icon"></i>
          ...
          <div class="row mb-2">
            ...
            <div class="col-sm-6">
              <ol class="breadcrumb float-sm-right">
                ...
                <li class="breadcrumb-item active">Airport Taxis</li>
              </ol>
            </div>
          </div>
        </div><!-- /.container-fluid -->
            ...
            <div class="col-12">
              ...
              <div class="card">
                <div class="card-header">
                  <h3 class="card-title">Airport Taxis Records</h3>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                  <table id="example1" class="table table-bordered table-striped">
                    ...
                    <tbody>
                      ...
                      $sales7 = "SELECT * FROM `airporttaxi` where DriverName='$places' order by IDs Desc";
                      ...
                      while ($row7 = mysqli_fetch_assoc($result7)) {
						...
						echo  "<td><form method='get' action='taxis.php'><button type='submit' name='offer'  value='" . $id . "' class='btn btn-block btn-warning'>Offer</button></form></td>";
					  }else{
						echo  "<td><form method='get' action='taxis.php'><button type='submit' name='notoffer'  value='" . $id . "' class='btn btn-block btn-warning'>Not Offered</button></form></td>";
					  }
						if($row7['Approved']=="Not Approved"){
						echo  "<td><form method='get' action='taxis.php'><button type='submit' name='approve'  value='" . $id . "' class='btn btn-block btn-warning'>Approve</button></form></td>";
					  }else{
						  echo  "<td style='overflow-x:scroll;'>" . $row7['Approved'] . "</td>";  
					  }
                        echo  "<td><form method='get' action='taxis.php'><button type='submit' name='delete'  value='" . $id . "' class='btn btn-block btn-danger'>Delete</button></form></td>";
                        echo " </tr>";
                        $counter3++;
                      }
					   }else{
					  $dates = date("Y-m-d");
                      require_once('../../connect/connect.php');
                      $sales7 = "SELECT * FROM `airporttaxi` order by IDs Desc";
                      $result7 = mysqli_query($link, $sales7);
                      $counter3 = 1;
                      $totalstock = 0;
                    ...
                    if (isset($_GET['approve'])) {
                      $id = $_GET['approve'];
                      $sales7 = "UPDATE  airporttaxi SET Approved='Approved' WHERE IDs='" . $id . "'";
                      $result = mysqli_query($link, $sales7);
                      if ($result) {
                        echo '<script type="application/javascript">';
                        echo 'alert("Taxi Record successfully Approved");';
                        echo 'window.location.href="taxis.php";';
                        echo '</script>';
                      } else {
                        echo '<script type="application/javascript">';
                        echo 'alert("Taxi Approval Failed");';
                        echo 'window.location.href="taxis.php";';
                        echo '</script>';
                      }
                    }
					 if (isset($_GET['offer'])) {
                      $id = $_GET['offer'];
                      $sales7 = "UPDATE  airporttaxi SET Offered='Offered' WHERE IDs='" . $id . "'";
                      $result = mysqli_query($link, $sales7);
                      if ($result) {
                        echo '<script type="application/javascript">';
                        echo 'alert("Taxi Record successfully Offered");';
                        echo 'window.location.href="taxis.php";';
                        echo '</script>';
                      } else {
                        echo '<script type="application/javascript">';
                        echo 'alert("Taxi Offer Failed");';
                        echo 'window.location.href="taxis.php";';
                        echo '</script>';
                      }
                    }
					if (isset($_GET['notoffer'])) {
                      $id = $_GET['notoffer'];
                      $sales7 = "UPDATE  airporttaxi SET Offered='Not Offered' WHERE IDs='" . $id . "'";
                      $result = mysqli_query($link, $sales7);
                      if ($result) {
                        echo '<script type="application/javascript">';
                        echo 'alert("Taxi offfer Record successfully Removed ");';
                        echo 'window.location.href="taxis.php";';
                        echo '</script>';
                      } else {
                        echo '<script type="application/javascript">';
                        echo 'alert("Taxi Offer Removal Failed");';
                        echo 'window.location.href="taxis.php";';
                        echo '</script>';
                      }
                    }
					if (isset($_GET['delete'])) {
                      $id = $_GET['delete'];
                      $sales7 = "DELETE  FROM airporttaxi WHERE IDs='" . $id . "'";
                      $result = mysqli_query($link, $sales7);
                      if ($result) {
                        echo '<script type="application/javascript">';
                        echo 'alert("Taxi Record successfully Deleted");';
                        echo 'window.location.href="taxis.php";';
                        echo '</script>';
                      } else {
                        echo '<script type="application/javascript">';
                        echo 'alert("Taxi Record Delete Failed");';
                        echo 'window.location.href="taxis.php";';
                        echo '</script>';
                      }
                    }
