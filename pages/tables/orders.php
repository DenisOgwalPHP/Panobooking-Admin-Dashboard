...
<head>
  ...
  <title>PANOBOOKING | Booking Records</title>
                  ...
                  <table id="example1" class="table table-bordered table-striped">
                    ...
                    <tbody>
                      ...
                      if (isset($_GET['ordertype'])) {
                        $ordertypes = $_GET['ordertype'];
                        if ($ordertypes == "new")
                          $dates = date("Y-m-d");;
                        require_once('../../connect/connect.php');
                        $sales7 = "SELECT * FROM `bookings` where Date like '$dates%' order by IDs Desc";
                        ...
                        while ($row7 = mysqli_fetch_assoc($result7)) {
                          ...
                          $id = $row7['OrderCode'];
                          echo  "<td>" . $counter3 . "</td>";
                          echo  "<td><a href='cart.php?ordercodes=" . $row7['OrderCode'] . "'>" . $row7['OrderCode'] . "</a></td>";
                          echo  "<td style='overflow-x:scroll;'>" . $row7['Date'] . "</td>";
                          echo  "<td style='overflow-x:scroll;'>" . $row7['TotalCost'] . "</td>";
                          echo  "<td style='overflow-x:scroll;'>" . $row7['User'] . "</td>";
                          $approvals = $row7['Cleared'];
                          if ($approvals == 'Yes') {
                            echo  "<td style='overflow-x:scroll;'>" . $row7['Cleared'] . "</td>";
                          } else {
                            echo '<td><form method="get" ><button type="submit" name="approve" value="' . $id . '" class="btn btn-block btn-warning">Clear</button></form></td>';
                          }

                          echo  "<td style='overflow-x:scroll;'>" . $row7['ClearanceType'] . "</td>";
                          echo  "<td><form method='get' action='orders.php'><button type='submit' name='delete'  value='" . $id . "' class='btn btn-block btn-danger'>Delete</button></form></td>";
                          echo " </tr>";
                          $counter3++;
                        }
                      } else {
                        require_once('../../connect/connect.php');
                        $sales7 = "SELECT * FROM `bookings` order by IDs Desc";
                        $result7 = mysqli_query($link, $sales7);
                        $counter3 = 1;
                        $totalstock = 0;
                    ...
                    if (isset($_GET['delete'])) {
                      $id = $_GET['delete'];
                      $sales7 = "DELETE  FROM cart WHERE OrderCode='" . $id . "'";
                      $results = mysqli_query($link, $sales7);

                      $sales6 = "DELETE  FROM bookings WHERE OrderCode='" . $id . "'";
                      $result = mysqli_query($link, $sales6);
                      if ($result) {
                        echo '<script type="application/javascript">';
                        echo 'alert("Order Record successfully Deleted");';
                        echo 'window.location.href="orders.php";';
                        echo '</script>';
                      } else {
                        echo '<script type="application/javascript">';
                        echo 'alert("Order Record Delete Failed");';
                        echo 'window.location.href="orders.php";';
                        echo '</script>';
                      }
                    }
                    ...
                    if (isset($_GET['approve'])) {
                      $id = $_GET['approve'];
                      if (isset($id)) {
                        $sales6 = "UPDATE  bookings SET Cleared='Yes' WHERE OrderCode='" . $id . "'";
                        $result = mysqli_query($link, $sales6);
                        if ($result) {
                          echo '<script type="application/javascript">';
