...
<head>
  ...
  <title>PANOBOOKING | Cart Records</title>
            ...
            <div class="col-12">
              ...
              <div class="card">
                <div class="card-header">
                  <h3 class="card-title">Cart Records</h3>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                  <table id="example1" class="table table-bordered table-striped">
                    <thead>
                      <tr>
						...
						<th>Booking Currency Value</th>
                        <th></th>
                      </tr>
                    </thead>
                    <tbody>
                      <?php
                      if (isset($_GET['ordercodes'])) {
                        $ordercodes = $_GET['ordercodes'];
                        $dates = date("Y-m-d");;
                        require_once('../../connect/connect.php');
                        $sales7 = "SELECT * FROM `cart` where OrderCode='$ordercodes' order by IDs Desc";
                        ...
                        while ($row7 = mysqli_fetch_assoc($result7)) {
                          ...
                          $id = $row7['IDs'];
                          echo  "<td>" . $counter3 . "</td>";
                          echo  "<td><a href=''>" . $row7['OrderCode'] . "</a></td>";
                          echo  "<td style='overflow-x:scroll;'>" . $row7['created_at'] . "</td>";
						  if($row7['BookingType']=="Stays"){
                          echo  "<td style='overflow-x:scroll;'><a href='destinations.php?places=".$row7['Facility']."'>" . $row7['Facility'] . "</a></td>";
						}else  if($row7['BookingType']=="Airport Taxi"){
                          echo  "<td style='overflow-x:scroll;'><a href='taxis.php?places=".$row7['Facility']."'>" . $row7['Facility'] . "</a></td>";
						}else  if($row7['BookingType']=="Car Rental"){
                          echo  "<td style='overflow-x:scroll;'><a href='cars.php?places=".$row7['Facility']."'>" . $row7['Facility'] . "</a></td>";
						}else  if($row7['BookingType']=="Ambulance"){
                          echo  "<td style='overflow-x:scroll;'><a href='ambulances.php?places=".$row7['Facility']."'>" . $row7['Facility'] . "</a></td>";
						}
                          ...
                          echo  "<td style='overflow-x:scroll;'><a href='registeredusers.php?singleuser=".$row7['User']."'>" . $row7['User'] . "</a></td>";
                          echo  "<td style='overflow-x:scroll;'>" . $row7['Booked'] . "</td>";
                          echo  "<td style='overflow-x:scroll;'>" . $row7['DatesFrom'] . "</td>";
                          echo  "<td style='overflow-x:scroll;'>" . $row7['DatesTo'] . "</td>";
                          echo  "<td style='overflow-x:scroll;'>" . $row7['Taken'] . "</td>";
                          echo  "<td style='overflow-x:scroll;'>" . $row7['Descriptions'] . "</td>";
                          echo  "<td style='overflow-x:scroll;'>" . $row7['Currency'] . "</td>";
						  echo  "<td style='overflow-x:scroll;'>" . $row7['CurrencyAmount'] . "</td>";
                          echo  "<td><form method='get' action='cart.php'><button type='submit' name='delete'  value='" . $id . "' class='btn btn-block btn-danger'>Delete</button></form></td>";
                          echo " </tr>";
                          $counter3++;
                        }
                      } else {
                        require_once('../../connect/connect.php');
                        $sales7 = "SELECT * FROM `cart` order by IDs Desc";
                        $result7 = mysqli_query($link, $sales7);
                        $counter3 = 1;
                        $totalstock = 0;
                        ...
                        while ($row7 = mysqli_fetch_assoc($result7)) {
                          ...
                          echo  "<td>" . $counter3 . "</td>";
                          echo  "<td><a href=''>" . $row7['OrderCode'] . "</a></td>";
                          echo  "<td style='overflow-x:scroll;'>" . $row7['Date'] . "</td>";
                          if($row7['BookingType']=="Stays"){
                          echo  "<td style='overflow-x:scroll;'><a href='destinations.php?places=".$row7['Facility']."'>" . $row7['Facility'] . "</a></td>";
						}else  if($row7['BookingType']=="Airport Taxi"){
                          echo  "<td style='overflow-x:scroll;'><a href='taxis.php?places=".$row7['Facility']."'>" . $row7['Facility'] . "</a></td>";
                          ...
                          echo  "<td style='overflow-x:scroll;'>" . $row7['DatesFrom'] . "</td>";
                          echo  "<td style='overflow-x:scroll;'>" . $row7['DatesTo'] . "</td>";
                          $approvals = $row7['Taken'];
                          if ($approvals == 'Yes') {
                            echo  "<td style='overflow-x:scroll;'>" . $row7['Taken'] . "</td>";
                          } else {
                            echo '<td><form method="get" ><button type="submit" name="approve" value="' . $id . '" class="btn btn-block btn-warning">Taken</button></form></td>';
                          }
                    ...
                    if (isset($_GET['delete'])) {
                      $id = $_GET['delete'];
                      $sales7 = "DELETE  FROM cart WHERE IDs='" . $id . "'";
                      $result = mysqli_query($link, $sales7);
                      if ($result) {
                        echo '<script type="application/javascript">';
                        echo 'alert("Cart Record successfully Deleted");';
                        echo 'window.location.href="cart.php";';
                        echo '</script>';
                      } else {
                        echo '<script type="application/javascript">';
                        echo 'alert("Cart Record Delete Failed");';
                        echo 'window.location.href="cart.php";';
                        echo '</script>';
                      }
                    }
                    ...
                    if (isset($_GET['approve'])) {
                      $id = $_GET['approve'];
                      if (isset($id)) {
                        $sales6 = "UPDATE cart SET Taken='Yes' WHERE IDs='" . $id . "'";
                        $result = mysqli_query($link, $sales6);
                        if ($result) {
                          echo '<script type="application/javascript">';
                          echo 'alert("Product Taken Successful");';
                          echo 'window.location.href="cart.php";';
                          echo '</script>';
                        } else {
                          echo '<script type="application/javascript">';
                          echo 'alert("Product Taken Failed");';
                          echo 'window.location.href="cart.php";';
                          echo '</script>';
                        }
                      }
