...
<head>
  ...
  <title>PANOBOOKING | Rental Cars Records</title>
          ...
          <div class="row mb-2">
            ...
            <div class="col-sm-6">
              <ol class="breadcrumb float-sm-right">
                ...
                <li class="breadcrumb-item active">Rental Cars</li>
              </ol>
            </div>
          </div>
        </div><!-- /.container-fluid -->
            ...
            <div class="col-12">
              ...
              <div class="card">
                <div class="card-header">
                  <h3 class="card-title">Rental Cars Records</h3>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                  <table id="example1" class="table table-bordered table-striped">
                    ...
                    <tbody>
                      ...
                      $sales7 = "SELECT * FROM `carrentals` where DriverName='$places' order by IDs Desc";
                      ...
                      while ($row7 = mysqli_fetch_assoc($result7)) {
						...
						echo  "<td><form method='get' action='cars.php'><button type='submit' name='offer'  value='" . $id . "' class='btn btn-block btn-warning'>Offer</button></form></td>";
					  }else{
						echo  "<td><form method='get' action='cars.php'><button type='submit' name='notoffer'  value='" . $id . "' class='btn btn-block btn-warning'>Not Offered</button></form></td>";
					  }
					  if($row7['Approved']=="Not Approved"){
						echo  "<td><form method='get' action='cars.php'><button type='submit' name='approve'  value='" . $id . "' class='btn btn-block btn-warning'>Approve</button></form></td>";
					  }else{
						  echo  "<td style='overflow-x:scroll;'>" . $row7['Approved'] . "</td>";  
					  }
                        echo  "<td><form method='get' action='cars.php'><button type='submit' name='delete'  value='" . $id . "' class='btn btn-block btn-danger'>Delete</button></form></td>";
                        echo " </tr>";
                        $counter3++;
                      }
					   }else{
                      $dates = date("Y-m-d");;
                      require_once('../../connect/connect.php');
                      $sales7 = "SELECT * FROM `carrentals` order by IDs Desc";
                      $result7 = mysqli_query($link, $sales7);
                      $counter3 = 1;
                      $totalstock = 0;
                    ...
                    <?php
					 if (isset($_GET['approve'])) {
                      $id = $_GET['approve'];
                      $sales7 = "UPDATE  carrentals SET Approved='Approved' WHERE IDs='" . $id . "'";
                      $result = mysqli_query($link, $sales7);
                      if ($result) {
                        echo '<script type="application/javascript">';
                        echo 'alert("Car Record successfully Approved");';
                        echo 'window.location.href="cars.php";';
                        echo '</script>';
                      } else {
                        echo '<script type="application/javascript">';
                        echo 'alert("cars Approval Failed");';
                        echo 'window.location.href="cars.php";';
                        echo '</script>';
                      }
                    }
					 if (isset($_GET['offer'])) {
                      $id = $_GET['offer'];
                      $sales7 = "UPDATE  carrentals SET Offered='Offered' WHERE IDs='" . $id . "'";
                      $result = mysqli_query($link, $sales7);
                      if ($result) {
                        echo '<script type="application/javascript">';
                        echo 'alert("Car Record successfully Offered");';
                        echo 'window.location.href="cars.php";';
                        echo '</script>';
                      } else {
                        echo '<script type="application/javascript">';
                        echo 'alert("cars Offer Failed");';
                        echo 'window.location.href="cars.php";';
                        echo '</script>';
                      }
                    }
					if (isset($_GET['notoffer'])) {
                      $id = $_GET['notoffer'];
                      $sales7 = "UPDATE  carrentals SET Offered='Not Offered' WHERE IDs='" . $id . "'";
                      $result = mysqli_query($link, $sales7);
                      if ($result) {
                        echo '<script type="application/javascript">';
                        echo 'alert("Car offfer Record successfully Removed ");';
                        echo 'window.location.href="cars.php";';
                        echo '</script>';
                      } else {
                        echo '<script type="application/javascript">';
                        echo 'alert("cars Offer Removal Failed");';
                        echo 'window.location.href="cars.php";';
                        echo '</script>';
                      }
                    }
                    if (isset($_GET['delete'])) {
                      $id = $_GET['delete'];
                      $sales7 = "DELETE  FROM carrentals WHERE IDs='" . $id . "'";
                      $result = mysqli_query($link, $sales7);
                      if ($result) {
                        echo '<script type="application/javascript">';
                        echo 'alert("Car Record successfully Deleted");';
                        echo 'window.location.href="cars.php";';
                        echo '</script>';
                      } else {
                        echo '<script type="application/javascript">';
                        echo 'alert("Car Record Delete Failed");';
                        echo 'window.location.href="cars.php";';
                        echo '</script>';
                      }
                    }
